    ...
    <el-col :span="24" class="main">
      <aside :class="collapsed ? 'menu-collapsed' : 'menu-expanded'">
        <el-menu :default-active="$route.path" class="el-menu-vertical-demo" @open="handleopen" @close="handleclose"
          ...
          <!--一级标题-->
          <template v-for="(item, index) in showItem" v-if="!item.hidden && !item.back">
            <el-menu-item :index="item.children[0].path" :key="item.children[0].path"
              v-if="item.children && !$store.state.child.length" align="center" @click="clickOne(item, showItem)">
          ...
          <template v-for="(item, index) in $store.state.child" v-if="$store.state.child.length">
            <el-menu-item @click="clickTWOS()" :index="item.path" :key="item.path"
              v-if="!item.hidden && item.name != '首页配置'" align="center">
              {{ item.name }}
            </el-menu-item>
            <el-menu-item @click="clickTWOS()" :index="item.path" :key="item.path"
              v-if="item.name == '首页配置' && (parkid || $route.query.id) == 3" align="center">
              {{ item.name }}
            </el-menu-item>
          </template>
        </el-menu>
