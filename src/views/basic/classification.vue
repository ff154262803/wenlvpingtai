<template>
  <div class="classificationPage">
    ...
    <div class="filter">
      <strong>分类：</strong>
      <span @click="changetype('')" :class="query.groupId == '' ? 'active' : ''"
        >不限</span
      >
      <span
        v-for="(item, index) in list"
        :key="index + ''"
        :class="query.groupId == item.toString() ? 'active' : ''"
    ...
    >
      ...
      <el-table-column prop="typeName" label="分类名" width="200">
      </el-table-column>
      <el-table-column label="类型">
        <template slot-scope="scope">{{
          typenamaedata[scope.row.groupId]
        }}</template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="150" align="center">
        <template slot-scope="scope">
          <el-button type="text" size="small" @click="edit(scope.row)"
            >修改</el-button
          >
          <el-button type="text" size="small" @click="UpDown(scope.row.id)"
            >删除</el-button
          >
        </template>
      </el-table-column>
  ...
  methods: {
    ...
    getAccessToken() {
      this.$ajax.queryTypeList(this.query, (res) => {
        ...
        if (res.totalPage < this.query.page) {
          //过滤时页数bug
          this.query.page = 1;
          this.getAccessToken();
        }
    ...
    submit(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          if (this.addData.id) {
            this.$ajax.updateType(
              {
                id: this.addData.id,
                parameters: {
                  typeName: this.addData.typeName,
                  groupId: this.addData.groupId,
                },
              },
              (res) => {
                this.$message({
