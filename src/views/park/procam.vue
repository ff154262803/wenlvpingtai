        ...
        <div class="el-dialog__wrapper" v-show="Addshow">
			<div class="el-dialog el-dialogadd">
				<div class="el-dialog__header">
					<span class="el-dialog__title">{{newdata.id?'修改全景图':'新增全景图'}}</span>
					...
					<el-form :model="newdata" :rules="rules" ref="newdata" label-width="120px">
						...
						<el-form-item label="场馆" prop="site">
							<el-select v-model="newdata.site"  >
								<el-option v-for="item in sitelist" :label="item.caption" :value="item.id" :key="item.id" ></el-option>
							</el-select>
						</el-form-item>
                        <el-form-item label="缩略图"  prop="picurl">
							...
							<div style="margin-top:20px">
								<img :src="$store.state.resip+newdata.picurl" alt="" class="pic" v-if="newdata.picurl" style="width:80px" ref="pic">
							</div>
						</el-form-item>
					</el-form>
				</div>
...
export default {
	...
	data() {
        return {
            ...
            newdata:{},
            rules: {
                caption: [{ required: true, message: '必填项'},{ max: 20, message: '最多20个字符', trigger: 'blur' }],
	...
	methods: {
        ...
        enableState(id,val){
			this.$ajax.setPanoramaEnableState({idlst:[id],isenable:val}, res => {
				this.$message({
					type: 'success',
					message: '设置成功!'
				});
        ...
        setenableState(val){
			if(this.multipleSelection.length>0){
				...
				this.$ajax.setPanoramaEnableState({idlst:idlst,isenable:val}, res => {
					this.$message({
						type: 'success',
						message: '设置成功!'
					});
		...
		beginshow(data){
			...
			if(data){
                ...
                this.fileList1=[{url:this.$store.state.resip+data.picurl,shortUrl:data.picurl}]
			}else{
				this.newdata={}
			}
		},
			...
			this.$refs[formName].validate((valid) => {
				if (valid) {
					if(this.newdata.id){
						...
						this.$ajax.updatePanorama({id: this.newdata.id,parameters: data}, res => {
							this.$message({
								type: 'success',
								message: '修改成功!'
							});
