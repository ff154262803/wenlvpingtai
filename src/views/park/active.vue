<template>
	<div>
		...
		<div class="filter">
			<el-button class="addBtn" type="primary" @click="beginshow()">添加活动</el-button>
		</div>
		<!-- 表格区 -->
		<el-table ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%"
			...
			<el-table-column label="操作" width="250">
				<template slot-scope="scope">
					<el-button type="text" size="small" @click="gotodetail(scope.row)"
						v-show="scope.row.issubscribe==1">预约管理</el-button>
					...
					<el-button type="text" size="small" @click="enableState(scope.row.id,1)"
						v-show="scope.row.isenable==0">启用</el-button>
					<el-button type="text" size="small" @click="enableState(scope.row.id,0)"
						v-show="scope.row.isenable==1">禁用</el-button>
				</template>
			</el-table-column>
		</el-table>
		<!--底部工具条-->
		...
		<div class="el-dialog__wrapper" v-show="Addshow">
			<div class="el-dialog el-dialogadd">
				<div class="el-dialog__header">
					<span class="el-dialog__title">{{newdata.id?'修改活动':'新增活动'}}</span>
					...
					<el-form :model="newdata" :rules="rules" ref="newdata" label-width="120px">
						...
						<el-form-item label="地点" prop="siteid">
							<el-select v-model="newdata.siteid">
								<el-option :label="n.caption" :value="n.id" :key="n.id" v-for="n in sitelist">
								</el-option>
							</el-select>
						</el-form-item>
						...
						<el-form-item label="开始结束时间" prop="time">
							<el-date-picker v-model="newdata.time" type="daterange" range-separator="至"
						...
						<el-form-item label="轮播图" prop="picurl">
							...
							<el-upload class="upload-demo" style="display:none" :on-remove="onremove" :data="uploaddata"
								:action="$store.state.ip+'/resources/uploadResource'" :on-progress="handleLoading"
								:on-success='onsuccsesspic' accept="image/jpeg,image/jpg,image/png"
								:before-upload="beforeUploadpic" :on-error="onerror" multiple list-type="picture-card">
						...
						<el-form-item label="缩略图" prop="thumbnail">
							...
							<el-upload class="upload-demo" style="display:none" :data="uploaddata"
								:action="$store.state.ip+'/resources/uploadResource'" :on-progress="handleLoading"
								accept="image/jpeg,image/jpg,image/png" :on-success='onsuccsess'
								:before-upload="beforeUploadpic" :on-error='onerror' list-type="picture">
								<el-button size="small" type="primary" id="uppict">点击上传</el-button>
							</el-upload>
							<div style="margin-top:20px">
								<img :src="$store.state.resip+newdata.thumbnail" alt="" class="pic"
									v-if="newdata.thumbnail" style="width:80px" ref="pic">
							</div>
						</el-form-item>
					</el-form>
				</div>
		...
		methods: {
			...
			setenableState(id, val) {
				if (this.multipleSelection.length > 0) {
					...
					this.$ajax.setEventsEnableState({ idlst: idlst, isenable: val }, res => {
						this.$message({
							type: 'success',
							message: '设置成功!'
						});
