        ...
        <el-table
            ...
            <el-table-column label="操作" width="250" >
				<template slot-scope="scope">
                    <el-button type="text" size="small" @click="gotodetail(scope.row)" v-show="scope.row.issubscribe==1">预约管理</el-button>
					...
					<el-form :model="newdata" :rules="rules" ref="newdata" label-width="120px">
						...
						<el-form-item label="地点" prop="siteid">
							<el-select v-model="newdata.siteid">
								<el-option  :label="n.caption" :value="n.id" :key="n.id" v-for="n in sitelist"></el-option>
							</el-select>
						</el-form-item>
                        ...
                        <el-form-item label="开始结束时间"  prop="time">
							<el-date-picker v-model="newdata.time" type="daterange" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期"></el-date-picker>
						</el-form-item>
				...
				if (valid) {
					...
					if(this.newdata.id){
						this.$ajax.updateEvents({id: this.newdata.id,parameters: {
							...
							siteid:this.newdata.siteid}}, res => {
							this.$message({
								type: 'success',
								message: '修改成功!'
							});
