  ...
  <div class="classificationPage">
    ...
    <div class="filter">
      ...
      <span
        ...
        :class="query.timeLimit == '' ? 'active' : ''"
        >不限</span
      >
      <span :class="query.timeLimit == 1 ? 'active' : ''" @click="changetype(1)"
        >1日</span
      >
      <span :class="query.timeLimit == 2 ? 'active' : ''" @click="changetype(2)"
        >2日</span
      >
      <span :class="query.timeLimit == 3 ? 'active' : ''" @click="changetype(3)"
        >3日</span
      >
      <span :class="query.timeLimit == 4 ? 'active' : ''" @click="changetype(4)"
        >1周</span
      >
      <span :class="query.timeLimit == 5 ? 'active' : ''" @click="changetype(5)"
        >1月</span
      >
    </div>
    <!--表格内容-->
    ...
    >
      ...
      <el-table-column prop="caption" label="用户名" width="120">
      </el-table-column>
      <el-table-column prop="mobile" label="账号" show-overflow-tooltip>
      </el-table-column>
      <el-table-column prop="system" label="注册平台" show-overflow-tooltip>
      </el-table-column>
      <el-table-column label="状态" width="80">
        <template slot-scope="scope">{{
          scope.row.isenable == 0 ? "冻结" : "启用"
        }}</template>
      </el-table-column>
      <el-table-column
        label="最后登录时间"
  ...
  methods: {
    enableState(val) {
      if (this.multipleSelection.length > 0) {
        ...
        this.$ajax.setWebUserEnableState(
    ...
    delAll() {
      if (this.multipleSelection.length > 0) {
        ...
        this.$confirm("您确定要删除选中用户吗?", "提示", {
          ...
          .then(() => {
            this.$ajax.deleteWebUser({ idlst: idlst }, (res) => {
              this.$message({
                type: "success",
                message: "删除成功!",
              });
