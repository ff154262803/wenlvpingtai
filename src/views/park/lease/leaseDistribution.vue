<template>
    <div class="classificationPage">
        ...
        <div class="filter">
            <strong>服务中心：</strong>
            <span @click="query.servicecenterid=''" :class='query.servicecenterid==""?"active":""'>不限</span>
            <span v-for="item in servicecenteridList"
                  :key="item.id"
                  :class='query.servicecenterid==item.id?"active":""'
                  @click="query.servicecenterid=item.id">{{item.caption}}</span>
        </div>
        <!--表格内容-->
        <el-table
            ref="multipleTable"
            ...
            <el-table-column
                prop="leaserescaption"
                label="物品">
            </el-table-column>
            <el-table-column
                prop="servicecentercaption"
                label="服务中心">
            </el-table-column>
            <el-table-column
                prop="unused"
        ...
        <!--弹窗内容-->
        <el-dialog title="添加分类" :visible.sync="addBol" :close-on-click-modal=false @close="addshow=true">
            <el-form :model="addData" :rules="rules" ref="addData">
                ...
                <el-form-item label="数量" label-width="120px" prop="unused">
                    <el-input v-model.number="addData.unused" style="width: 200px"></el-input>
                </el-form-item>
                <el-button type="text" v-if="!addshow">该地点的物品已经存在</el-button>
            </el-form>
            <div slot="footer" class="dialog-footer">
                <el-button @click="addshow=true,addBol = false">取 消</el-button>
                <el-button type="primary" @click="subAdd('addData')" v-if="send==''&& addshow">确 定</el-button>
                <el-button type="primary" @click="submit('addData')" v-if="send=='change'">修 改</el-button>
            </div>
        </el-dialog>
    </div>
</template>
        ...
        data() {
            return {
                ...
                rules: {
                    ...
                    unused: [{type: 'number', required: true, message: '请输入数量', trigger: 'blur'}]
                },
                addData: {},
                tableData: [],
                multipleSelection: []
