    ...
    >
      ...
      <el-table-column label="状态">
        <template slot-scope="scope">{{
          scope.row.isenable == "0" ? "禁用" : "可用"
        }}</template>
      </el-table-column>
      <el-table-column label="赔偿条款">
        <el-button
          type="text"
          size="small"
          slot-scope="scope"
          @click="checkH5(scope.row)"
          >查看</el-button
        >
      </el-table-column>
      <el-table-column label="操作" width="150" align="center">
        <template slot-scope="scope">
          <el-button type="text" size="small" @click="isEnable(scope.row)"
            >{{ scope.row.isenable == "0" ? "启用" : "禁用" }}
          </el-button>
          <el-button
            type="text"
            size="small"
            @click="DetailBolChange(scope.row)"
            >条款</el-button
          >
    ...
    <div class="el-dialog__wrapper" v-show="DetailBol">
      <div class="el-dialog el-dialogedit">
        <div class="el-dialog__header">
          <span class="el-dialog__title">编辑条款</span>
          <button
            class="el-dialog__headerbtn"
            aria-label="Close"
            type="button"
            @click="DetailBol = false"
          >
...
export default {
  ...
  methods: {
    ...
    isEnable(isEnable) {
      ...
      this.$ajax.setLeaseResEnableState(
        { idlst: [isEnable.id], isenable: enable },
        (res) => {
          this.queryLeaseResList();
        }
      );
