<template>
    <div class="classificationPage">
        ...
        <el-button icon="el-icon-search" class="btn" @click="queryLostPropertyList"></el-button>
        <el-button class="addBtn" type="primary" @click="addBtn">添加失物</el-button>
        <el-button class="addBtn" type="primary" @click="mustKnow">领取须知</el-button>
        <div class="filter">
            <strong>分类：</strong>
            <span @click="changetype('')" :class='query.typelist==""?"active":""'>不限</span>
            <span v-for="item in list"
                  :key="item.id"
                  :class='~query.typelist.indexOf(item.id)?"active":""'
                  @click="changetype(item.id)">{{item.typeName}}</span>
        </div>
        <div class="filter">
            <strong>时间：</strong>
            <el-date-picker
        ...
        <div class="filter">
            ...
            <span @click="query.status='1'" :class='query.status=="1"?"active":""'>已认领</span>
            <span @click="query.status='0'" :class='query.status=="0"?"active":""'>未认领</span>
        </div>
        <!--表格内容-->
        <el-table
            ref="multipleTable"
            ...
            <el-table-column
                label="状态">
                <template slot-scope="scope">{{ scope.row.status=='0'?'未认领':'已认领' }}</template>
            </el-table-column>
            <el-table-column
                prop="createtime"
                label="创建时间">
            ...
            <el-table-column label="操作" width="150" align="center">
                <template slot-scope="scope">
                    <el-button type="text" size="small" @click="detailLost(scope.row)">详情</el-button>
        ...
        <el-dialog :title="['添加失物','修改失物','失物详情'][changeBol]" v-if="addBol" :visible.sync="addBol"
            ...
            <el-form :model="addData" :rules="rules" ref="addData">
                <el-form-item :label="['分类名','失物简述','失物简述'][changeBol]" label-width="120px" prop="caption">
                ...
                <el-form-item label="分类" label-width="120px" prop="type">
                    <el-select v-model="addData.type" placeholder="请选择分类">
                        <el-option :label='item.typeName' :value="item.id" v-for="(item, index) in list"
                ...
                <el-form-item label="图片：" label-width="120px" prop="picurl">
                    ...
                    <div style="margin-top:20px">
                        <div class="pic" v-for="(n,i) in fileList" :key="n" v-if="fileList.length">
            ...
            <el-form :model="claimData" :rules="rules" ref="claimData" v-if="claimerBol" style="overflow: hidden">
                <div class="el-dialog__header">
                    <span class="el-dialog__title">认领人信息</span>
                </div>
                <el-form-item label="姓名" label-width="120px" prop="claimName">
                    <el-input v-model="claimData.claimName" style="width: 200px"></el-input>
                </el-form-item>
                <el-form-item label="联系方式" label-width="120px" prop="claimPhone">
                    <el-input v-model="claimData.claimPhone" style="width: 200px"></el-input>
                </el-form-item>
                <el-form-item label="证件" label-width="120px" prop="claimCard">
                    <el-input v-model="claimData.claimCard" style="width: 200px"></el-input>
                </el-form-item>
                <el-form-item style="float: right">
                        <el-button @click="cancel()">取 消</el-button>
                        <el-button type="primary" @click="claim('claimData')">确 定</el-button>
                </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer" v-if="!claimerBol">
                <el-button @click="addBol = false" v-if="!detailLostBol">取 消</el-button>
                <el-button type="primary" @click="submit('addData')" v-if="!detailLostBol">确 定</el-button>
                <el-button type="primary" @click="claimClick()" v-else>认 领</el-button>
            </div>
        </el-dialog>
        <!--领取须知-->
        <el-dialog title="领取须知" :visible.sync="mustKnowBol" :close-on-click-modal=false>
            <el-form :model="mustKnowData" :rules="rules" ref="mustKnowData">
                ...
                <el-form-item label="领取地点" label-width="120px" prop="address">
                    <el-input v-model="mustKnowData.address" style="width: 200px"></el-input>
                    <el-button type="primary" plain icon="el-icon-location-outline" class="btn"
                               @click="mapBol=true"></el-button>
                </el-form-item>
                <el-form-item label="咨询电话" label-width="120px" prop="number">
                    <el-input v-model="mustKnowData.number" style="width: 200px"></el-input>
                </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
                <el-button @click="mustKnowBol = false">取 消</el-button>
                <el-button type="primary" @click="mustKnowSubmit('mustKnowData')">确 定</el-button>
            </div>
        </el-dialog>
        <!--地图-->
        <div class="el-dialog__wrapper" v-show="mapBol">
            <div class="el-dialog el-dialogadd">
                <div class="el-dialog__header">
                    ...
                    <button class="el-dialog__headerbtn" aria-label="Close" type="button" @click="mapBol=false"><i
                        class="el-dialog__close el-icon el-icon-close"></i></button>
                </div>
                <div class="el-dialog__body">
                    <div style="height: 400px" id="myMap"></div>
                    <el-input type="text" id="tipinput" v-model="tipinput" placeholder="请输入您想查询的位置"
    ...
    export default {
        ...
        data() {
            ...
            return {
                ...
                rules: {
                    caption: [{required: true, message: '请输入失物简述', trigger: 'blur'}, {max: 50, message: '最多50个字符', trigger: 'blur'}],
                    type: [{required: true, message: '请选择失物分类', trigger: 'change'}],
                    content: [{required: true, message: '请输入失物描述', trigger: 'blur'}, {max: 500, message: '最多500个字符', trigger: 'blur'}],
                    // content1: [{required: true, message: '请输入未认领失物描述', trigger: 'blur'}, {max: 500, message: '最多500个字符', trigger: 'blur'}],
                    address: [{required: true, message: '请输入领取地点', trigger: 'blur'}, {max: 50, message: '最多50个字符', trigger: 'blur'}],
                    number: [{required: true,validator: checkPhone, trigger: 'blur'}],//{required: true,message: '请输入电话', trigger: 'blur'},
                    claimPhone: [{required: true,validator: checkPhone, trigger: 'blur'}],
        ...
        methods: {
            //查询失物的分类
            queryTypeList() {
                this.$ajax.queryTypeList({condition: "", count: 999, groupId: "9", page: 1}, res => {
                    this.list = res.data;
            ...
            //改变分类
            changetype(val) {
                if (val) ~this.query.typelist.indexOf(val) ? this.query.typelist.splice(this.query.typelist.indexOf(val), 1) : this.query.typelist.push(val);
            ...
            submit(formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        if (this.addData.id) {
                            this.$ajax.updateLostProperty({id: this.addData.id, parameters: this.addData}, res => {
                                this.$message({
                                    type: 'success',
                                    message: '修改成功!'
                                });
            ...
            mustKnow() {
                this.$ajax.getParkInstructions({id: sessionStorage.getItem('parkid')}, res => {
                    this.mustKnowData = res.data ? res.data : {parkid: sessionStorage.getItem('parkid')};
