    ...
    >
      ...
      <el-table-column label="状态">
        <template slot-scope="scope">{{
          scope.row.soldstatus == 0 ? "下架" : "上架"
        }}</template>
      </el-table-column>
      <el-table-column label="剩余/总数量">
        <template slot-scope="scope"
          >{{ scope.row.currentnum }}/{{ scope.row.num }}</template
        >
      </el-table-column>
      ...
      <el-table-column label="操作" width="155">
        <template slot-scope="scope">
          ...
          <el-button
            ...
            v-show="scope.row.autosold == 1 && scope.row.soldstatus == 0"
            >上架</el-button
          >
          <el-button
            type="text"
            ...
            v-show="scope.row.autosold == 1 && scope.row.soldstatus == 1"
            >下架</el-button
          >
          <el-button
            type="text"
        ...
        <div class="el-dialog__body">
          ...
          >
            ...
            <el-form-item
              label="上架状态"
              prop="soldstatus"
              v-show="newdata.autosold == 1"
            >
              <el-radio-group v-model="newdata.soldstatus">
                <el-radio v-model="newdata.soldstatus" label="0" checked
                  >下架</el-radio
                >
...
export default {
  ...
  methods: {
        ...
        this.$ajax.addH5(this.h5, (res) => {
          if (res.resb == 200) {
            ...
            this.$ajax.updateMallGoods(
              { id: this.newdata.id, parameters: { linkh5url: linkh5url } },
              (res) => {
                this.$message({
                  type: "success",
                  message: "修改成功!",
    ...
    enableState(id, val) {
      this.$ajax.updateMallGoods(
        { id: id, parameters: { soldstatus: val } },
        (res) => {
          this.$message({
            type: "success",
            message: "设置成功!",
